...
class MdxSecuritySimulator(models.Model):
    ...
    def _parse_domain(self):
        ...
        try:
            ...
            expression.expression(domain, self.env[self.model].sudo())
        except Exception as error:
    ...
    def _check_acl(self, model, operation):
        try:
            return bool(model.check_access_rights(operation, raise_exception=False)), False
        except Exception as error:
    ...
    def _check_record_rule(self, model, operation, record):
        try:
            model.browse(record.id).check_access_rule(operation)
    ...
    def _safe_filter_rules(self, model, records, operation):
        try:
            return model.browse(records.ids)._filter_access_rules_python(operation)
        except Exception:
            return model.browse()
    ...
    def _rule_record_result(self, rule, operation, record):
        ...
        if rule.groups and not (rule.groups & self.user_id.groups_id):
            return "not_applicable"
        if not record:
            return "not_checked"
        try:
            domain = safe_eval(rule.domain_force, self._eval_context()) if rule.domain_force else []
            domain = expression.AND([[("id", "=", record.id)], expression.normalize_domain(domain)])
    ...
    def _build_group_values(self):
        ...
        return [
            {
                "group_id": group.id,
                "category": group.category_id.display_name or "Uncategorized",
                "used_by_acl": group in acl_groups,
                "used_by_rule": group in rule_groups,
                "technical_name": self._xmlid_for(group),
            }
            for group in self.user_id.groups_id.sorted(
                lambda group: (group.category_id.display_name or "", group.name or "")
            )
        ]
            ...
            values.append(
                {
                    ...
                    "group_names": ", ".join(action.groups_id.mapped("display_name")),
                    "accessible": bool(not action.groups_id or action.groups_id & user_groups),
