...
class TestAccessEngine(TransactionCase):
    ...
    def test_group_label_is_application_qualified(self):
        """Odoo 18 has no res.groups.privilege; groups hang off ir.module.category."""
        category = self.env["ir.module.category"].create({"name": "MDX Test App"})
        group = self.Groups.create({"name": "Manager", "category_id": category.id})
        self.assertEqual(self.Engine._group_label(group), "MDX Test App / Manager")
