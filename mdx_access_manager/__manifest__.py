...
Built for the way Odoo 18 organises security: groups sit under applications,
and the roles you assign rarely match the access a user ends up with. Every
report names the application alongside the group, because a bare group name
("User", "Manager") is ambiguous in exactly the places it matters.

Odoo 18 writes implied groups straight onto the user record, so the user form
shows one flat list with no sign of which entries were chosen and which arrived
by inheritance. This module recovers that distinction from the implication
graph and tells you which groups the user holds only because another one
implies them.

Effective rights matrix
    Every model a user can reach, with read / write / create / delete resolved
    the way the server resolves it, and which of the user's groups were
