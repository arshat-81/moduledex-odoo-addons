...
possible: a real user is just ``user.groups_id`` expanded, and a
...
* ``res.groups.trans_implied_ids`` is the transitive closure of
  ``implied_ids`` and is NOT reflexive - it excludes the group itself - so the
  closure is ``groups | groups.trans_implied_ids``. Odoo 18 also materialises
  implication onto the user: ``res.groups.write()`` inserts the closure into
  ``res_groups_users_rel``, so ``groups_id`` already holds every implied group
  and no stored trace of the original human choice survives.
"""
...
class MdxAccessEngine(models.AbstractModel):
    ...
    def _expand_groups(self, groups):
        ...
        ``trans_implied_ids`` excludes the group itself, unlike Odoo 19's
        ``all_implied_ids``, so the originals are unioned back in. Accepts a
        recordset or an iterable of ids.
        """
        ...
        return groups | groups.trans_implied_ids

    @api.model
    def _user_group_sets(self, user):
        """Return ``(stored, effective)`` group recordsets for a user.

        On Odoo 18 these are normally the same set, because implication is
        materialised onto ``groups_id`` when a group is granted. The expansion
        is still applied: it costs nothing and it keeps the result correct on a
        database where the two have drifted apart (a restore, a manual SQL
        insert, or a module that wrote the relation directly).
        """
        user = user.sudo()
        stored = user.groups_id
        return stored, self._expand_groups(stored)

    @api.model
    def _inherited_groups(self, groups):
        """Groups the user holds only because another of their groups implies it.

        Odoo 18's user form shows one flat list with no indication of which
        entries a human actually ticked and which arrived by implication. This
        recovers that distinction from the implication graph.
        """
        return groups.filtered(
            lambda g: g in (groups - g).trans_implied_ids
        )
    ...
    def _implication_paths(self, explicit_groups, target_group):
        ...
        for explicit in explicit_groups:
            ...
            elif target_group in explicit.trans_implied_ids:
    ...
    def _group_label(self, group):
        """Application-qualified group name.

        Odoo 18 has no ``res.groups.privilege``; groups carry ``category_id``
        pointing at ``ir.module.category``, which is what the user form groups
        them under. A bare group name ("User", "Manager") is ambiguous in
        exactly the places this module reports, so the application is prefixed.
        """
        if not group:
            return ""
        category = group.category_id.name or ""
        return "%s / %s" % (category, group.name) if category else group.name
