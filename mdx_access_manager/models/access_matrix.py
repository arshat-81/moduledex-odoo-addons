...
class MdxAccessMatrix(models.Model):
    ...
    # Odoo 18 materialises implied groups onto the user, so there is no stored
    # "assigned" set to contrast with an "effective" one. The useful split here
    # is which of the stored groups arrived by implication.
    explicit_group_ids = fields.Many2many(
        "res.groups", "mdx_access_matrix_explicit_rel", "matrix_id", "group_id",
        string="Groups on User", readonly=True,
        help="Every group on the user record. Odoo 18 writes implied groups here too, so this "
             "list mixes what was chosen with what was inherited.",
    )
    effective_group_ids = fields.Many2many(
        "res.groups", "mdx_access_matrix_effective_rel", "matrix_id", "group_id",
        string="Inherited Groups", readonly=True,
        help="The groups above that the user holds only because another of their groups implies "
             "them. Odoo's user form gives no indication of which these are.",
    )
    inherited_group_count = fields.Integer(string="Inherited", readonly=True)
    ...
    def action_run(self):
        ...
        for matrix in self:
            ...
            inherited = Engine._inherited_groups(effective)
            ...
            if matrix.model_filter or not matrix.include_transient:
                ...
                if not model_names:
                    matrix.write({
                        "explicit_group_ids": [(6, 0, effective.ids)],
                        "effective_group_ids": [(6, 0, inherited.ids)],
                        "inherited_group_count": len(inherited),
            ...
            note = self.env._(
                "Resolved %(models)s model(s) from %(groups)s group(s), %(inh)s of them inherited.",
                models=len(perms), groups=len(effective), inh=len(inherited))
