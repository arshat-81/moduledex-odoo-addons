...
class MdxAccessChangeSet(models.Model):
    ...
    remove_group_ids = fields.Many2many(
        ...
        help="Removing a group that another of the user's groups implies will not take effect: "
             "Odoo re-adds it. Remove it at the source instead.",
    )
    ...
    def _proposed_explicit_groups(self):
        ...
        self.ensure_one()
        current = self.user_id.sudo().groups_id
    ...
    def action_apply(self):
        ...
        for rec in self:
            ...
            before = rec.user_id.sudo().groups_id
            ...
            rec.user_id.write({"groups_id": [(6, 0, proposed.ids)]})
            ...
            self.env["mdx.access.change.log"].sudo()._record(
                ...
                before={"groups_id": sorted(before.mapped("name"))},
                after={"groups_id": sorted(proposed.mapped("name"))},
                change_set=rec,
            )
            rec.write({
                "state": "applied",
