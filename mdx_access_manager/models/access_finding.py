...
class MdxAccessFinding(models.Model):
    ...
    def _detect_redundant_implication(self):
        ...
        ``trans_implied_ids`` is the transitive closure either way. Removing it
        ...
        for group in groups:
            ...
            for target in direct:
                ...
                if not others:
                    continue
                if target in others.trans_implied_ids:
                    via = others.filtered(lambda g, t=target: t in g.trans_implied_ids)
                    out.append({
    ...
    def _detect_admin_path(self):
        ...
        for group in self.env["res.groups"].sudo().search([("id", "!=", admin.id)]):
            if admin not in group.trans_implied_ids:
                continue
